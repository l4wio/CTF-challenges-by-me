<title>Tester MySQL - Test 2</title>
<style>
body {
font-family:Tahoma;
...
$count = mysql_query('SELECT count FROM basic WHERE id='.$_GET['id']) or die (mysql_error()); 
while($arraycount = mysql_fetch_array($count)) {
$arraycount['count']++;
$countupdate = intval($arraycount['count']);
break;
}
$update = mysql_query('UPDATE basic SET count='.$countupdate.' WHERE id='.$_GET['id']);
